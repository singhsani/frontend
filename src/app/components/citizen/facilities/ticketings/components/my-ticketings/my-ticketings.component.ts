...
import { BookingUtils } from '../../../bookings/config/booking-config';
  ...
  ticketingConstants = TicketingConstants;
	...
	chooseForCancel(shift: any, checked) {
		if (checked) {
			...
			if (!data && (shift.status === this.ticketingConstants.DEPOSIT_REQUIRED || shift.status === this.ticketingConstants.BOOKED)) {
	...
	getAllSelected(): boolean {
		return (this.CancelRequestList.length ==
			this.CancelSlotList.filter(shift => shift.status == this.ticketingConstants.DEPOSIT_REQUIRED ||
				shift.status == this.ticketingConstants.SUBMITTED ||
				shift.status == this.ticketingConstants.PAYMENT_REQUIRED ||
				shift.status == this.ticketingConstants.BOOKED).length) ||
			(this.CancelSlotList.filter(shift => shift.status == this.ticketingConstants.CANCELLATION_REQUEST ||
				shift.status == this.ticketingConstants.CANCELLATION_APPROVED).length === this.CancelSlotList.length);
	}
	...
	getAllDisabled(): boolean {
		return this.CancelSlotList.filter(shift => shift.status == this.ticketingConstants.CANCELLATION_REQUEST || shift.status == this.ticketingConstants.CANCELLATION_APPROVED).length == this.CancelSlotList.length;
	}
			...
			merge(this.sort.sortChange, this.paginator.page)
				.pipe(
					...
					switchMap(() => {
						...
						this.ticketingService.resourceType = this.searchBookingsForm.get('resourceType').value;
						this.ticketingService.pageIndex = (this.paginator.pageIndex + 1);
						this.ticketingService.pageSize = this.paginator.pageSize;
						this.getAllLookUP();
						return this.ticketingService!.getAllTicketing(this.searchBookingsForm.get('refNumber').value);//NOSONAR
					}),
					map(data => {
