...
@Component({
	selector: 'app-food-duplicate',
	templateUrl: './food-duplicate.component.html',
	styleUrls: ['./food-duplicate.component.scss']
})
export class FoodDuplicateComponent implements OnInit {

	foodDuplicateForm: FormGroup;
	translateKey: string = 'foodDuplicateScreen';
	...
	ngOnInit() {
		...
		this.getLookupData();
		this.foodDuplicateFormControls();
   ...
   */
	createRecordPatchSerachData(searchData: any) {
		...
		this.formService.createFormData().subscribe(res => {
			...
			this.foodDuplicateForm.patchValue(searchData);

			this.foodDuplicateForm.patchValue({
				id: res.id,
				...
				// licenseDuplicatealDate: res.licenseDuplicatealDate,
	...
	getFoodLicNewData() {
		this.formService.getFormData(this.formId).subscribe(res => {
			try {
				this.foodDuplicateForm.patchValue(res);
				this.showButtons = true;
			} catch (error) {
	...
	*/
	getLookupData() {
		this.formService.getDataFromLookups().subscribe(res => {
			...
			this.onChangeZone(this.foodDuplicateForm.get('firmZone').value.code);
		});
	}
   ...
   */
	onChangeRegOrLicType(event) {
		if (event == 'LESS_THAN_12LK') {
			this.foodDuplicateForm.get('regOrLic').setValue('Registration');
			this.foodDuplicateForm.get('regOrLic').disable();
		}
		if (event == 'GREATER_THAN_12LK') {
			this.foodDuplicateForm.get('regOrLic').setValue('License');
			this.foodDuplicateForm.get('regOrLic').disable();
	...
	*/
	foodDuplicateFormControls() {
		this.foodDuplicateForm = this.fb.group({
