...
import { ShopAndEstablishmentCloseService } from '../common/services/shop-and-establishment-close.service';
import { BookingConstants } from '../../../facilities/bookings/config/booking-config';
import { identity } from 'rxjs';
@Component({
  selector: 'app-shop-lic-close',
  templateUrl: './shop-lic-close.component.html',
  styleUrls: ['./shop-lic-close.component.scss']
})
export class ShopLicCloseComponent implements OnInit {

  @ViewChild('postalAddressEstablishment') postalAddressEstablishment: any;

	shopLicCloseForm: FormGroup;
	...
	translateKey: string = 'shopLicCloseScreen';
	...
	constructor(
		...
		private shopAndEstablishmentCloseService : ShopAndEstablishmentCloseService,
	) { }
  ...
  calculateWorkers(indx) {
	...
	hideGuideLine(flag: boolean) {
		...
		console.log("Regis ", this.registrationType);
			...
			this.shopAndEstablishmentCloseService.getLatestApplicationsByNumber(this.certificateNumber).subscribe(res => {
				console.log('Res', res)
	...
	setFormDataFromLatestApplication(res){
		...
		this.shopLicCloseForm.patchValue	({
		...
		let formObj = this.shopLicCloseForm.getRawValue();
		...
		formObj['closingCertificateNumber'] =  this.certificateNumber
		...
		this.formService.saveFormData(formObj).subscribe(saveResp => {
			this.shopLicCloseForm.patchValue(saveResp);
	...
	getShopLicNewData() {
		...
		this.formService.getFormData(this.formId).subscribe(res => {
			try {
				this.shopLicCloseForm.patchValue(res);
				...
				debugger;
				if(res.closingCertificateNumber) {
					this.shopLicCloseForm.get('closingCertificateNumber').disable();
					this.isGuideLineActive = false;
				}

				this.certificateNumber = res.closingCertificateNumber;
			} catch (error) {
	...
	setDropdownAndListDataFromRes(res){
		this.licenseConfiguration.isAttachmentButtonsVisible = true;
				...
				// if (this.shopLicCloseForm.get('ownershipType').value) {
				// 	this.updateServiceUploadDocument(this.shopLicCloseForm.get('ownershipType').value)
				// }
				...
				res.serviceDetail.serviceUploadDocuments.forEach(app => {
					(<FormArray>this.shopLicCloseForm.get('serviceDetail').get('serviceUploadDocuments')).push(this.licenseConfiguration.createDocumentsGrp(app));
				});
	...
	*/
	shopLicNewFormControls() {
		this.shopLicCloseForm = this.fb.group({
    ...
    //   certificateNumber: [null],
		...
		this.attachmentdetail = this.fb.group({
			...
			/*  */
			closingCertificateNumber : [null,Validators.required],
		});
		//this.addMorePerson('EMPLOYER_FAMILY');
		//this.addMorePerson('OCCUPANCY');

		// this.shopLicCloseForm.get('zone').valueChanges.subscribe(data => {
		// 	console.log(this.wardZoneLevel1List)
		// 	// this.shopLicCloseForm.get('waterDrainageZoneId').setValue(data);
		// });
		// this.shopLicCloseForm.get('ward').valueChanges.subscribe(data => {
		// 	this.shopLicCloseForm.get('waterDrainageWardId').setValue(data);
		// });
		// this.shopLicCloseForm.get('block').valueChanges.subscribe(data => {
		// 	this.shopLicCloseForm.get('waterDrainageBlockId').setValue(data);
		// });
		this.commonService.createCloneAbstractControl(this.establishmentdetails,this.shopLicCloseForm);
		this.commonService.createCloneAbstractControl(this.employerdetails,this.shopLicCloseForm);
		this.commonService.createCloneAbstractControl(this.employerfamily,this.shopLicCloseForm);
		this.commonService.createCloneAbstractControl(this.personoccuping,this.shopLicCloseForm);
		this.commonService.createCloneAbstractControl(this.partnerlist,this.shopLicCloseForm);
		this.commonService.createCloneAbstractControl(this.attachmentdetail,this.shopLicCloseForm);	
	...
	requiredDocumentList() {
		...
		// this.uploadFilesArray = [];
		// let organizationCategory = this.shopLicCloseForm.get('organizationType').value.code;
		// if (organizationCategory) {
		// 	_.forEach(this.shopLicCloseForm.get('serviceDetail').get('serviceUploadDocuments').value, (value) => {
	...
	createArrayWorkOut(data?: any): FormGroup {
		return this.fb.group({
			...
			noOfMen: [data.noOfMen ? data.noOfMen : 0,{validators:[Validators.required,Validators.min(0) ,
			]}],
			noOfWomen: [data.noOfWomen ? data.noOfWomen : 0,{validators:[Validators.required,Validators.min(0) ,
			]}],
...
// frist time
	...
	dateFormat(date, controlType: string) {
		this.shopLicCloseForm.get(controlType).setValue(moment(date).format("YYYY-MM-DD"));
	...
	calulateNumberOfPerson(formType: string, fieldsType: string, filterType: string) {
		let countNumber = [];
		let data = (<FormArray>this.shopLicCloseForm.get(formType)).controls;
		if (data.length) {
			switch (filterType) {
			...
			this.shopLicCloseForm.get(fieldsType).setValue(countNumber.length);
	...
	*/
	onChangeNoOfHumanWorking(event) {
		...
		this.shopLicCloseForm.get('registrationType').setValue(event);
	...
	*/
	onChangeTypeOfOrganization(event) {
		...
		try {
			...
			this.shopLicCloseForm.get('attachments').setValue([]);
	...
	getTotalEmployeePerson() {
		let totalAdultEmployee = this.shopLicCloseForm.get('totalAdultEmployee').value || 0;
		let totalYoungEmployee = this.shopLicCloseForm.get('totalYoungEmployee').value || 0;
		let totalManEmployee = this.shopLicCloseForm.get('totalManEmployee').value || 0;
		let totalWomenEmployee = this.shopLicCloseForm.get('totalWomenEmployee').value || 0;
		let totalUnidentified = this.shopLicCloseForm.get('totalUnidentified').value || 0;
		...
		this.shopLicCloseForm.get('totalEmployee').setValue(totalcount);
		return totalcount;
	}
	...
	setValidationReq(formControlName: string) {
		if (this.shopLicCloseForm.get('applicantVimaAmountPaid').get('code').value == 'YES') {
			this.shopLicCloseForm.get(formControlName).setValidators([Validators.required, Validators.maxLength(20)]);
		}
		else {
			this.shopLicCloseForm.get(formControlName).clearValidators();
		}
		this.shopLicCloseForm.get(formControlName).updateValueAndValidity();
	}

	patchValue() {
		this.shopLicCloseForm.patchValue(this.dummyJSON);
	}
	...
	updateServiceUploadDocument(ownershipType,organizationCode) {
		let array = (<FormArray>this.shopLicCloseForm.get('serviceDetail').get('serviceUploadDocuments'));
		...
		// 				file['mandatory'] = false;
		// 			}
		// 			(<FormArray>this.shopLicCloseForm.get('serviceDetail').get('serviceUploadDocuments')).push(this.licenseConfiguration.createDocumentsGrp(file));
		// 		}
		// 		break;
		// 	default:
	...
	commonUploadDocument(){
		const comonDocument = [
			...
			{
				documentIdentifier: 'PROPERTY_TAX_RECEIPT',
				mandatory: true
			},
			{
				documentIdentifier: 'SHOP_CLOSING_PHOTO',
				mandatory : true
			},
			{
				documentIdentifier: 'APPLICANT_PHOTO',
				mandatory : true
			},
			{
			...
			{
				documentIdentifier: 'OTHER_DOC1',
				mandatory: false
			},
			{
				documentIdentifier: 'OTHER_DOC2',
				mandatory: false
			},
			{
				documentIdentifier: 'OTHER_DOC3',
				mandatory: false
			},
			{
