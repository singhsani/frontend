...
export class NumberOnlyDirective {
  ...
  // How to use
  // <input number-only [allowDecimals]="true" [allowSign]="true" type="text" >


  @Input() allowDecimals: boolean = true;
  @Input() allowSign: boolean = false;
  @Input() onlyTwoDecimal: boolean = true;
  ...
  @HostListener('keydown', ['$event']) onKeyDown(e: KeyboardEvent) {
    ...
    let allowedKeys = [
      'Backspace', 'ArrowLeft', 'ArrowRight', 'Escape', 'Tab', 'Home','Delete', 'Ctrl'
    ];
    ...
    if (this.onlyTwoDecimal) {
      ...
      if (next && !String(next).match(new RegExp(/^\d*\.?\d{0,2}$/g))) {
        event.preventDefault();
      }
    }
    else {
