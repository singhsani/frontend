...
@Component({
  selector: 'app-intimation-to-registration',
  templateUrl: './intimation-to-registration.component.html',
  styleUrls: ['./intimation-to-registration.component.scss']
})
export class IntimationToRegistrationComponent implements OnInit {
  ...
  */
  ...
  setFormDataFromLatestApplication(res) {
    ...
    formObj['intimationtocertificationNumber'] = this.certificateNumber
  ...
  getShopLicNewData() {
    ...
    this.formService.getFormData(this.formId).subscribe(res => {
      try {
        ...
        if (res.intimationtocertificationNumber) {
          this.shopLicTransferForm.get('intimationtocertificationNumber').disable();
          this.isGuideLineActive = false;
        }

        this.certificateNumber = res.intimationtocertificationNumber;
    ...
    if(res.dateofhavingmorethan10Emp != null){
      this.intiToCre = true;
    }else{
  ...
  */
  shopLicNewFormControls() {
    ...
    this.personoccuping = this.fb.group({
      ...
      dateofhavingmorethan10Emp: [null, Validators.required]
    });
    ...
    this.attachmentdetail = this.fb.group({
      attachments: [''],
      /* */
      intimationtocertificationNumber: [null, Validators.required],
    });
  ...
  */
  ...
  dateFormats(date, controlType: string) {
    this.personoccuping.get(controlType).setValue(moment(date).format("YYYY-MM-DD"));
  }
  ...
  */
  ...
  commonUploadDocument() {
    const comonDocument = [
      ...
      {
				documentIdentifier: 'OTHER_DOCUMENT_2',
				mandatory: false
			},
			{
				documentIdentifier: 'OTHER_DOCUMENT_1',
				mandatory: false
			},
			{
				documentIdentifier:'PROFESSIONAL_TAX_RECEIPT_FORM_5',
				mandatory: true
			},
			{
				documentIdentifier:'PAYROLL_REGISTER_COPY',
				mandatory: true
			},
      {
        documentIdentifier: 'FORM_K',
        mandatory: true
      }
    ];
  ...
  */
  ...
  ischeckmorethan10Emp() {
    let controls=this.personoccuping.get('workerCounts')['controls'].length;
    let men=0;
    let women = 0;
    let totalEmp = 0;
    for(let i=0;i<controls;i++){
      totalEmp  += this.personoccuping.get('workerCounts')['controls'][i].value.total;
    }

    if(totalEmp > 9){
      this.intiToCre = true;
      this.personoccuping.get("dateofhavingmorethan10Emp").setValidators([Validators.required]);


    }else{
      this.intiToCre=false;
      this.personoccuping.get("dateofhavingmorethan10Emp").reset();
      this.personoccuping.get("dateofhavingmorethan10Emp").clearValidators();
    }
  }
  getCommonWorkerType() {
    let workerGrid = <FormArray>this.personoccuping.get('workerCounts');
